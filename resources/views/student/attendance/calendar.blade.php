...
@section('title', 'Kalendarz frekwencji')
                ...
                <div class="calendar-grid">
                    ...
                    <div class="calendar-body">
                        ...
                        @for($week = 0; $week < 6; $week++)
                            @for($dow = 0; $dow < 7; $dow++)
                                @if(($week == 0 && $dow < $currentDay) || $day > $daysInMonth)
                                ...
                                @else
                                    ...
                                    @endphp
                                    <div class="calendar-cell {{ $isToday ? 'today' : '' }} {{ $isWeekend ? 'weekend' : '' }} {{ $dayAttendances->isNotEmpty() ? 'has-attendance' : '' }}"
                                         @if($dayAttendances->isNotEmpty()) data-bs-toggle="tooltip" data-bs-html="true" title="@foreach($dayAttendances as $att)<div class='text-start'><span class='badge bg-{{ $att->status == 'present' ? 'success' : ($att->status == 'late' ? 'warning' : ($att->status == 'absent' ? 'danger' : 'secondary')) }} me-1'>{{ $att->status == 'present' ? '✓' : ($att->status == 'late' ? '⏰' : ($att->status == 'absent' ? '✗' : '?')) }}</span>{{ $att->subject->name }}</div>@endforeach" @endif>
                                        <div class="day-number {{ $isToday ? 'bg-primary text-white' : '' }}">{{ $day }}</div>
                                        <div class="day-content">
                                            @if($dayAttendances->isNotEmpty())
                                                <div class="attendance-dots">
                                                    @foreach($dayAttendances->take(4) as $att)
                                                        <span class="attendance-dot {{ $att->status }}"></span>
                                                    @endforeach
                                                    @if($dayAttendances->count() > 4)
                                                        <span class="more-indicator">+{{ $dayAttendances->count() - 4 }}</span>
                                                    @endif
                                                </div>
                                                <div class="attendance-summary">
                                                    @if($hasPresent)
                                                        <span class="summary-badge present">{{ $hasPresent }}</span>
                                                    @endif
                                                    @if($hasLate)
                                                        <span class="summary-badge late">{{ $hasLate }}</span>
                                                    @endif
                                                    @if($hasAbsent)
                                                        <span class="summary-badge absent">{{ $hasAbsent }}</span>
                                                    @endif
                                                </div>
                                            @endif
                                        </div>
    ...
    <div class="col-md-3 mb-3">
        <div class="card border-left-success shadow h-100">
            <div class="card-body">
                ...
                @endphp
                <div class="h5 mb-0 font-weight-bold">{{ $monthPresent }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-3">
        <div class="card border-left-warning shadow h-100">
            <div class="card-body">
                ...
                @endphp
                <div class="h5 mb-0 font-weight-bold">{{ $monthLate }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-3">
        <div class="card border-left-danger shadow h-100">
            <div class="card-body">
                ...
                @endphp
                <div class="h5 mb-0 font-weight-bold">{{ $monthAbsent }}</div>
            </div>
        </div>
    </div>
    <div class="col-md-3 mb-3">
        <div class="card border-left-primary shadow h-100">
            <div class="card-body">
                ...
                @endphp
                <div class="h5 mb-0 font-weight-bold">{{ $monthRate }}%</div>
            </div>
        </div>
    </div>
</div>
